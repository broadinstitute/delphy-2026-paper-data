...
def compare_logs(
    ...
    def extract_ml_skyline():
        ts, lo, median, hi = [], [], [], []
        for t, (Ne, lower, upper) in sorted(ml_results['skyline'].items()):
            ts.append(t)
            lo.append(lower)
            median.append(Ne)
            hi.append(upper)
        return (ts, lo, median, hi)
    ...
    if ml_results:
        [ts, lo, median, hi] = extract_ml_skyline()
        ax1.fill_between(ts, lo, hi, color='red', alpha=0.2)
        ax1.plot(ts, median, color='red', linestyle='-')
        
    ax1.set_yscale('log')
    pos = ax1.get_position()
    new_pos = [pos.x0 + 0.03, pos.y0, pos.width - 0.03, pos.height]
    ax1.set_position(new_pos)
    #ax1.margins(x=0.6, y=0.1)
    ax1.set_xlim((bt.decimalDate("2023-10-01"), bt.decimalDate("2025-08-05")))  # ???: A little bit less on the right to align with tree
...
# Non-alpha datasets
ml_results = extract_ml_results('ml/h5n1_andersen.iqtree', 'ml/tt', False)
delphy_a_log = Log(
    filename='delphy_outputs_a/h5n1-andersen-e756a15-ALL_full_dates_only_a.log',
...
delphy_b_log = Log(
    filename='delphy_outputs_b/h5n1-andersen-e756a15-ALL_full_dates_only_b.log',
...
# Alpha datasets
ml_results_alpha = extract_ml_results('ml/h5n1_andersen_alpha.iqtree', 'ml/tt_alpha', True)
delphy_alpha_a_log = Log(
    filename='delphy_outputs_alpha_a/h5n1-andersen-e756a15-ALL_full_dates_only_alpha_a.log',
...
delphy_alpha_b_log = Log(
    filename='delphy_outputs_alpha_b/h5n1-andersen-e756a15-ALL_full_dates_only_alpha_b.log',
