...
def compare_logs(log_and_wallclock_minutes: list[tuple[Log, float]],
    ...
    for (ess_table, (log, wallclock_minutes)) in zip(ess_tables, log_and_wallclock_minutes):
        max_skygrid_ess = max(ess_table[f'skygrid.logPopSize{i+1}'] for i in range(23))
        min_skygrid_ess = min(ess_table[f'skygrid.logPopSize{i+1}'] for i in range(23))
...
compare_logs(
    [(delphy_a_log,    37 +  6.238/60.0),
     (beastX_a_log, 30449 + 13.490/60.0),],  # Cut short at ~5300 samples
    'plots/ESSDelphyAVsBeastXA.pdf'
)
compare_logs(
    [(delphy_b_log,    37 + 23.493/60.0),
     (beastX_b_log, 30440 + 58.869/60.0),],  # Cut short at ~5300 samples
...
compare_logs(
    [(delphy_alpha_a_log,    42 + 10.540/60.0),
     (beastX_alpha_a_log, 30436 + 11.470/60.0),],  # Cut short at ~3800 samples
    'plots/ESSDelphyAVsBeastXAAlpha.pdf'
)
compare_logs(
    [(delphy_alpha_b_log,    42 + 22.719/60.0),
     (beastX_alpha_b_log, 30430 + 55.540/60.0),],  # Cut short at ~3700 samples
