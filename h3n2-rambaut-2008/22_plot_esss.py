...
def compare_logs(log_and_wallclock_minutes: list[tuple[Log, float]],
    ...
    for (ess_table, (log, wallclock_minutes)) in zip(ess_tables, log_and_wallclock_minutes):
        ...
        this_esss = {
            ...
            'Skygrid logPopSize (max)': max_skygrid_ess,
            'Skygrid logPopSize (min)': min_skygrid_ess,
...
compare_logs(
    [(delphy_a_log,     3 +  9.023/60.0),
     (beastX_a_log,    91 + 41.341/60.0),],
    'plots/ESSDelphyAVsBeastXA.pdf'
)
compare_logs(
    [(delphy_b_log,     3 +  9.010/60.0),
     (beastX_b_log,    92 + 17.375/60.0),],
...
compare_logs(
    [(delphy_alpha_a_log,     3 + 32.376/60.0),
     (beastX_alpha_a_log,   153 + 32.690/60.0),],
    'plots/ESSDelphyAVsBeastXAAlpha.pdf'
)
compare_logs(
    [(delphy_alpha_b_log,     3 + 30.848/60.0),
     (beastX_alpha_b_log,   153 +  5.034/60.0),],
