...
def compare_logs(
        ...
        show_alpha=False,
        ml_results={},
        only_alphas: list[Log] = []
):

    only_alphas_filenames = [log.filename for log in only_alphas]
    ...
    def make_subplot(i, j, colname, title, adjuster=lambda x: x):
        ...
        for log, data, ess_table in zip(logs, datasets, ess_tables):
            ...
            #if colname != 'gammaShape' and (log.filename in only_alphas_filenames):
            #    continue

            is_only_alpha = (log.filename in only_alphas_filenames)
            ...
            bw_adjust = (n / ess) ** (1./5.)
            this_ax = sb.kdeplot(adjuster(data[log_colname]), fill=not is_only_alpha, color=log.color, bw_adjust=bw_adjust,
                                 ls='-' if not is_only_alpha else ('dotted' if log.filename == only_alphas[0].filename else 'dashed'));
...
# Alpha datasets
ml_results_alpha = extract_ml_results('ml/ma_sars_cov_2_alpha.iqtree', 'ml/tt_alpha', True)
...
compare_logs(
    [delphy_alpha_a_log, beast2_alpha_log, beastX_alpha_log, beastX_alpha_2cats_log, beastX_alpha_8cats_log],
    'plots/DelphyAVsBeast2VsBeastXAlpha.pdf',
    burnin=0.30,
    ml_results=ml_results_alpha,
    show_alpha=True,
    only_alphas=[beastX_alpha_2cats_log, beastX_alpha_8cats_log]
)
compare_logs(
    [delphy_alpha_b_log, beast2_alpha_log, beastX_alpha_log, beastX_alpha_2cats_log, beastX_alpha_8cats_log],
