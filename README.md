...
Although these scripts were last successfully run in early 2026 to prepare, execute and
analyze the Delphy, BEAST X and BEAST2 runs, there is no guarantee that they will run perfectly in
...
Some files in this repo (notably `*.trees` and `*.dphy` run results file) were too big to
push to GitHub.  Except for clones of public GitHub repos, the full contents are available
at Zenodo
[![here](https://zenodo.org/badge/DOI/10.5281/zenodo.19819574.svg)](https://doi.org/10.5281/zenodo.19819574).
...
Compile the associated Rust analysis programs (you need a Rust installation):
```
cd tree_ess
cargo build --release
cd ..
```
This builds three binaries: `calc-tree-ess`, `compare-clades`, and `calc-clade-coverage`.
...
calc-tree-ess (built from tree_ess/)
compare-clades (built from tree_ess/)
calc-clade-coverage (built from tree_ess/)
```
...
- Delphy Version 1.1.4 (build 2044, commit a50e378) - benchmarks
- Delphy Version 1.3.1 (build 2051, commit a4769da) and Delphy Version 1.3.2 (build 2052, commit d05ddb8) - WCSSs
- MAFFT Version 7.505 (2022/Apr/10)
- BEAST2 v2.7.7
- BEAGLE commit `6480ad3` (Mon Sep 15 2025)
- BEAST X 10.5.0
- Sapling Version 0.1.1 (build 2, commit a0b9da1) - `sims` benchmark
- Sapling Version 0.2.0 (build 7, commit 5b5dcc7) - WCSSs
- IQ-TREE 2.3.6
- TreeTime 0.11.4
- calc-tree-ess, compare-clades, calc-clade-coverage 0.1.0 (in this repository, under `tree_ess/`)
...
- 177 mpox sequences (197,209 sites), almost all from Nigeria, mostly preceding the 2021-2022 worldwide mpox outbreak
...
## sars-cov-2-gisaid-week-by-week

- Growing subsets of GISAID SARS-CoV-2 sequences (29,903 sites), collected on or before 2020-03-31, run week-by-week by collection date and by submission date
- Data: GISAID (cannot be shared publicly; see [sars-cov-2-gisaid-week-by-week/README.md](sars-cov-2-gisaid-week-by-week/README.md))
- Run with all-default parameters

## h5n1-daily-updated-tree

- Automated daily Delphy runs on the latest H5N1 genotype B3.13 cattle outbreak data from the Andersen lab's `avian-influenza` repo
- Data: https://github.com/andersen-lab/avian-influenza (latest commit)
- See [h5n1-daily-updated-tree/README.md](h5n1-daily-updated-tree/README.md) for setup

## sims

- Simulated datasets generated by Sapling under constant and exponential population models, with 100, 1000, 10000 and 100000 tips (L = 30,000 sites)
- Used to benchmark Delphy's runtime scaling and accuracy against ML methods (IQ-TREE)
- Run under both constant and exponential population models


# Well-Calibrated Simulation Studies (WCSS)

The `wcss/` directory contains a series of well-calibrated simulation studies that validate
Delphy's MCMC inference, following the methodology of Mendes, Bouckaert, Carvalho & Drummond
(2025).  Each study simulates many datasets by drawing parameters from Delphy's priors and
generating sequence data with Sapling, then runs Delphy on each dataset and checks that the
true generating values are recovered at the expected rates.

See [wcss/README.md](wcss/README.md) for the full list of studies and details.
