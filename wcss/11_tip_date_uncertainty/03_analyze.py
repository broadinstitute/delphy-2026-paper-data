    ...
    with open(maple_path) as f:
        for line in f:
            if not line.startswith(">"):
                continue
            # Line format: >TIP_042|2025-07-15
...
def compute_hpd(samples, credible_mass=0.95):
    """Compute shortest interval containing credible_mass of sorted samples."""
...
def analyze_tip_dates(script_dir, analyses_dir, included, burnin_frac):
    ...
    for j, i in enumerate(included):
        ...
        # Find age(...) columns for uncertain tips
        age_cols = [c for c in data.columns if c.startswith("age(")]
        if not age_cols:
            continue
        ...
        for col in age_cols:
            # Parse tip name and date format from column name
            # Column format: age(TIP_XXX|YYYY-MM) or age(TIP_XXX|YYYY)
            inner = col[4:-1]  # strip "age(" and ")"
            ...
            if len(date_part) == 7:  # YYYY-MM
                uncertainty_type = "month"
            elif len(date_part) == 4:  # YYYY
                uncertainty_type = "year"
            else:
                continue
            ...
            normalized_rank = rank / len(posterior_calendar_years)
            posterior_mean = np.mean(posterior_calendar_years)
            hpd_lo, hpd_hi = compute_hpd(posterior_calendar_years)
