...
def check_ess(la_df, analyses_dir, ignore_low_ess=False,
    ...
    # Filter out columns with NaN values (e.g., tip-date ESS when no
    # tips of that type exist)
    ess_cols = [c for c in ess_cols if la_df[c].notna().all()]
    n = len(la_df)
...
def run_tip_loganalyser(script_dir, n, burnin_pct):
    ...
    Returns:
        tip_la_results: list mapping replicate index to list of
            (tip_prefix, uncertainty_type, mean, hpd_lo, hpd_hi, ess)
        min_tip_ess_month: list mapping replicate index to min ESS across
            month-uncertain tips (NaN if no month-uncertain tips)
        min_tip_ess_year: list mapping replicate index to min ESS across
            year-uncertain tips (NaN if no year-uncertain tips)
    """
    tip_la_results = [None] * n
...
def main():
    ...
    # `excluded` contains row indices into la_df (0..n-1)
    ...
    if excluded:
        ...
        with open(excl_path, "w") as f:
            ...
            for row_idx in sorted(excluded):
                low_obs = []
                for col in ess_cols:
                    obs_name = col[:-4]
                    if obs_name in ESS_IGNORE:
                        continue
                    if la_df[col].iloc[row_idx] < ESS_THRESHOLD_VERY_LOW:
                        low_obs.append(obs_name)
                f.write(f"sim_{row_idx:03d}\t{','.join(low_obs)}\n")
