...
def plot_clock_rates(delphy_logs, beast_logs, out_pdf_filename):
    fig,ax = plt.subplots(figsize=(4.5,4),facecolor='w')
    plt.tight_layout()
...
def plot_doubling_time_dist(delphy_logs, beast_logs, out_pdf_filename):
    fig,ax = plt.subplots(figsize=(4,2.5),facecolor='w')
    plt.tight_layout()

    for (logs, color, linestyle, label, yoffset) in [(delphy_logs, 'green', '-', '(Delphy)', 0.2),
                                                    (beast_logs, 'blue', 'dotted', '(BEAST X)', 0.0)]:
        ...
        plt.text(1.9+mean_t_dbl, 1.5+yoffset, f'Mean {label}: {mean_t_dbl:.2f} years', horizontalalignment='center')
...
for run in ('a', 'b'):
    ...
    beast_logs = extract_log_params(f'beastX_run/Mpox_2poch_combined.log', burnin=0.3)
